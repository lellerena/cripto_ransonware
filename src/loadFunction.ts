    ...
    rl.question('Enter the directory containing encryption results: ', async (directory) => {
        try {
            if (encryptionResultsExist(directory)) {
                ...
                console.log(`Total data encrypted: ${encryptionResults.encryptionStats.totalSize / (1024 * 1024)} MB`);
                console.log(`Encryption timestamp: ${encryptionResults.encryptionStats.timestamp}`);
                console.log(
